...
import Post from 'duroom/forum/components/Post';
...
import humanTime from 'duroom/common/utils/humanTime';
...
export default function () {
  ...
  Post.prototype.flagActionItems = function () {
    ...
    items.add(
      ...
      <Button className="Button" icon="far fa-eye-slash" onclick={this.dismissFlag.bind(this)}>
        {app.translator.trans('duroom-flags.forum.post.dismiss_flag_button')}
      </Button>,
      -100
    );
  ...
  Post.prototype.flagReason = function (flag) {
    if (flag.type() === 'user') {
      ...
      const reason = flag.reason() ? app.translator.trans(`duroom-flags.forum.flag_post.reason_${flag.reason()}_label`) : null;
      const detail = flag.reasonDetail();
      const time = humanTime(flag.createdAt());

      return [
        app.translator.trans(reason ? 'duroom-flags.forum.post.flagged_by_with_reason_text' : 'duroom-flags.forum.post.flagged_by_text', {
          time,
          user,
          reason,
        }),
